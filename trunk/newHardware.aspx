...
void popInfo(Object s, EventArgs e) 
{
	...
	sql="SELECT * FROM (SELECT hwClassName FROM hwTypes WHERE hwType='Chassis') AS a WHERE hwClassName='"+v_type+"'";
	dat=readDb(sql);
	if (!emptyDataset(dat))
	{
		Response.Write("<script>window.location.href='newBladeCenter.aspx?redir=1&dc="+dcArg+"';<"+"/script>");
	}
	sql="SELECT * FROM (SELECT hwClassName FROM hwTypes WHERE hwClassName LIKE 'P%') AS a WHERE hwClassName='"+v_type+"'";
	dat=readDb(sql);
	if (!emptyDataset(dat))
	{
		Response.Write("<script>window.location.href='newPseries.aspx?redir=1&dc="+dcArg+"';<"+"/script>");
	}
	if (v_type=="new")
	{
		formHwClass.Value="";
	...
	else
	{
		...
		if (!emptyDataset(dat))
		{
			if (dat.Tables[0].Rows[0]["hwType"].ToString()=="Switch")
			{
				modelName.InnerHtml="Name:";
				note.InnerHtml="<SPAN class='bold'>NOTE: Click 'Save Changes' only once -- building the switch will take a bit of time...</SPAN>";
			}
		}
		else
		{
...
//			Response.Write(v_slot);
	...
	{
		sql="SELECT MAX(slot) FROM "+dcArg+"rackspace WHERE rack='0000'";
		dat=readDb(sql);
		if (!emptyDataset(dat))
		{
			string nextSlot="";
			...
			{
				case "MSJET":
					nextSlot=dat.Tables[0].Rows[0]["expr1000"].ToString();
				break;
				case "MSSQL":
					nextSlot=dat.Tables[0].Rows[0]["Column1"].ToString();
				break;
			}
//			string nextSlot=dat.Tables[0].Rows[0]["expr1000"].ToString();
			if (nextSlot==null || nextSlot=="")
			{
				nextSlot="00";
			}
			topU=Convert.ToInt32(nextSlot);
			topU++;
			if (topU<10)
			{
				v_slot="0"+topU.ToString();
			}
			else
			{
				v_slot=topU.ToString();
			}
		}
		else
		{
...
//		Response.Write(sql);
		...
		if (sqlSuccess) 
		{
			...
			else
			{
				...
				{
					Response.Write("<script>Progress.innerText='Building Switch...';<"+"/script>");
					sql="SELECT rackspaceId FROM "+dcArg+"rackspace WHERE model='"+v_model+"'";
					dat=readDb(sql);
					string rackId="";
					if (!emptyDataset(dat))
					{
					...
					if (rackId!="")
					{
						...
						case "Catalyst 6509 Switch":
							sqlErr=buildCisco6509(v_model,rackId,dcArg);
							break;
						case "DS9509-EMC":
							sqlErr=buildDS9509(v_model,rackId,dcArg);
							break;
						case "Catalyst 3750G":
							sqlErr=buildCatalyst3750G(v_model,rackId,dcArg);
							break;
						case "Nexus 5020":
							sqlErr=buildNexus5020(v_model,rackId,dcArg);
							break;
						case "Nexus 2232PP":
							sqlErr=buildNexus2232PP(v_model,rackId,dcArg);
							break;
						case "Nexus 2248TP":
							sqlErr=buildNexus2248TP(v_model,rackId,dcArg);
							break;
						case "MDS 9120":
							sqlErr=buildMDS9120(v_model,rackId,dcArg);
							break;
						case "MDS 9020":
							sqlErr=buildMDS9020(v_model,rackId,dcArg);
							break;
						case "Nexus 7000":
							sqlErr=buildNexus7000(v_model,rackId,dcArg);
							break;
						}
						...
						{
							sqlSuccess=false;
							errmsg.InnerHtml = "SQL Update Error - Switch Table Creation<BR/>"+sqlErr;
						}
						else
						{
							sqlErr="";
			...
			Response.Write("<script>frm.goButton.disabled=false;<"+"/script>");
			if (sqlErr==null || sqlErr=="")
			{
...
void goClick(Object s, EventArgs e)
{
	Response.Write("goClick");	
}

public void Page_Load(Object o, EventArgs e)
{
//UNCOMMENT this line to force login to access this page.
	lockout(); Page.Header.Title=shortSysName+": Add New Hardware";
	...
	try
	{
		req=unFixQueryStr(Request.QueryString["req"].ToString());	
	}
	catch (System.Exception ex)
	{
		req="";
	...
	titleSpan.InnerHtml=dcArg.ToUpper().Replace("_","")+" "+req;
	if (!IsPostBack)
	{
		if (req.ToLower()=="switch")
		{
			sql="SELECT * FROM hwTypes WHERE hwType='Switch' ORDER BY hwClassName ASC";
		}
		else
		{
		...
		sql="SELECT rackId, location FROM "+dcArg+"racks ORDER BY rackId ASC";
		racksdd=readDb(sql);
//		Response.Write(sql+"<BR/>");
		...
		if (req!="") // Find switches in rackspace, but not configured as a switch for porting.
		{
			if (req=="Switch")
			{
				sql="SELECT model, class, rack, slot, rackspaceId FROM (SELECT * FROM "+dcArg+"rackspace WHERE rackspaceId NOT IN (SELECT "+dcArg+"switches.rackspaceId from "+dcArg+"switches LEFT JOIN "+dcArg+"rackspace on "+dcArg+"rackspace.rackspaceId="+dcArg+"switches.rackspaceId)) AS a WHERE class IN (SELECT hwClassName FROM hwTypes WHERE hwType='Switch')";
				dat=readDb(sql);
				if (!emptyDataset(dat))
				{
					quickStr="<TABLE style='border-collapse: collapse;'><TR><TH style='border-width: 1px; padding: 1px; border-style: inset; border-color: black; background-color: black; font-size: 70%; color: white;' class='center'><SPAN class='bold'>Switch</SPAN></TH><TH style='border-width: 1px; padding: 1px; border-style: inset; border-color: black; background-color: black; font-size: 70%; color: white;' class='center'><SPAN class='bold'>Type</SPAN></TH><TH style='border-width: 1px; padding: 1px; border-style: inset; border-color: black; background-color: black; font-size: 70%; color: white;' class='center'><SPAN class='bold'>Location</SPAN></TH><TH style='border-width: 1px; padding: 1px; border-style: inset; border-color: black; background-color: black; font-size: 70%; color: white;' class='center'>&#xa0;</TH></TR>";
					foreach(DataRow dr in dat.Tables[0].Rows)
					{
						quickStr=quickStr+"<TR><TD style='border-width: 1px; padding: 1px; border-style: solid; border-color: black;'>&#xa0; "+dr["model"].ToString()+" &#xa0;</TD><TD style='border-width: 1px; padding: 1px; border-style: solid; border-color: black;'>&#xa0; "+dr["class"].ToString()+" &#xa0;</TD><TD style='border-width: 1px; padding: 1px; border-style: solid; border-color: black;'>&#xa0; Rack:"+dr["rack"].ToString()+", U"+dr["slot"].ToString()+" &#xa0;</TD><TD style='border-width: 1px; padding: 1px; border-style: solid; border-color: black;'>&#xa0; <A href='javascript:{}' onclick=javascript:window.open('addExistingSwitch.aspx?id="+dr["rackspaceId"].ToString()+"&dc="+dcArg+"','addExistSwitchWin','width=400,height=350,menubar=no,status=yes,scrollbars=yes') ALT='Add Existing Switch' class='nodec'>Add</A>&#xa0;</TD></TR>";
					}
					quickStr=quickStr+"</TABLE>";
				}
				quickAdd.InnerHtml=quickStr;
			}
		}
	}
...
<!--#include file='popup_opener.inc' -->
	<DIV id='popContent'>
		...
		&nbsp;<BR/>
			<DIV class='center'>
				<BR/>
				<DIV id='quickAdd' runat='server'/>
				<BR/>
				<SELECT id='formExistType' runat='server'><OPTION value='new' selected='true'>Add New Type ...</OPTION></SELECT>
					...
					<TR>
						<TD class='center'>
							<TABLE>
								...
								<TR>
									<TD class='inputform'>Server/Appliance?:</TD>
									<TD class='whiteRowFill left'>
				...
				<INPUT type='button' id='submitButton' value='Save Changes' name='goButton' OnServerClick='goSubmit' runat='server' />&#xa0;<INPUT type='button' value='Cancel &amp; Close' onclick='refreshParent()'>
				<BR/><BR/>
				<DIV id='note' class='statusLine' runat='server'/><BR/>
			</DIV>
		&nbsp;<BR/>
		</DIV>
	</DIV> <!-- End: popContent -->
